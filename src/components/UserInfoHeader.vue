<template>
  <Card r-radius l-radius class="rounded-tr-none rounded-tl-none">
    <div class="flex justify-between flex-wrap">
      <div class="flex items-center md:w-1/3 w-full">
        <Avatar :src='this.data?.avatar' size='lg' />
        <div class="ml-4">
      ...
      <div
        ...
        <p class="font-montBold text-2xl my-2">
          {{ data?.balance }}

        </p>
        <p class="font-mont text-md">
          Available: {{ data?.availableBalance }}
        </p>
      </div>
      <div class="text-right md:w-1/3 w-full flex flex-col justify-center items-center md:items-end">
        <Button title="Pay Out Now" type="secondary" extra-classes="w-max" />
        <p class="font-mont text-md mt-4 cursor-pointer">View Payments on Stripe</p>
      </div>
    </div>
  </Card>
</template>
